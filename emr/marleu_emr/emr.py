...
def __get_running_apps_ids(master_dns):
    running_apps_uri = "http://{}:{}/ws/v1/cluster/apps/?state=running".format(
        master_dns, YARN_PORT
    )
    # TODO handle errors?
...
def __list_steps(region, cluster_id, states):
    client = boto3.client("emr", region_name=region)
    return client.list_steps(ClusterId=cluster_id, StepStates=states)
...
def cancel_pending_steps(cluster_id, region):
    print(
        "Killing pending steps for cluster {} in region {}...".format(
            cluster_id, region
        )
    )
    ...
    if step_ids:
        client.cancel_steps(ClusterId=cluster_id, StepIds=step_ids)
...
def __getSshKey(env, region, key_pair_credstash_key):
    ...
    return credstash.getSecret(
        name=key_pair_credstash_key, table=credstash_store, region=region
    )
...
def kill_running_apps(env, region, cluster_id, key_pair_credstash_key):
    print(
        "Killing running apps for cluster {} in region {}...".format(cluster_id, region)
    )
...
def get_parser():
    ...
    parsers["super"].add_argument(
        "-c", "--cluster-id", default=None, required=True, help="cluster id"
    )

    subparsers = parsers["super"].add_subparsers(help='Try "emr command --help"')

    action = "cancel-pending-steps"
    parsers[action] = subparsers.add_parser(
        action, help="cancel all the pending emr steps"
    )
    parsers[action].set_defaults(action=action)

    action = "kill-running-apps"
    parsers[action] = subparsers.add_parser(
        action, help="kill all the running emr applications"
    )
    parsers[action].set_defaults(action=action)
    ...
    parsers[action] = subparsers.add_parser(
        action,
        help="cancel all the pending emr steps and then "
    ...
    parsers[action] = subparsers.add_parser(
        action, help="run a command on the emr master node through SSH"
    )
    parsers[action].set_defaults(action=action)
    __add_env_credstash_args(parsers[action])
    parsers[action].add_argument(
        "-x",
        "--command",
        default=None,
        required=True,
        help="command to execute on master node " "through SSH connection",
    )
